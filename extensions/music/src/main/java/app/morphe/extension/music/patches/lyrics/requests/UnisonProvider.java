...
public final class UnisonProvider implements LyricsProvider {
    ...
    private Lyrics fetchByVideoId(String videoId, String title, String artist,
        ...
        try {
            ...
            Logger.printDebug(() -> "Unison: GET /lyrics?v=" + videoId
                    + " keyId=" + getKeyId().substring(0, 8) + "…");
            ...
            Logger.printDebug(() -> "Unison: video id lookup failed", ex);
            return null;
        } finally {
            if (connection != null) {
                connection.disconnect();
    ...
    private static String computeKeyId() {
        try {
            ...
            Logger.printDebug(() -> "Unison: identity generation failed, using fallback key id");
    ...
    private Lyrics parseLyrics(String format, String lyrics) {
        switch (format.toLowerCase(Locale.ROOT)) {
            case "ttml":
                final TtmlParser.TtmlResult result = TtmlParser.parse(lyrics);
