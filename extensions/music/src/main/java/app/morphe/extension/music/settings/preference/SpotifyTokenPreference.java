...
public class SpotifyTokenPreference extends Preference {

    public SpotifyTokenPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init();
    }

    public SpotifyTokenPreference(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public SpotifyTokenPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SpotifyTokenPreference(Context context) {
        super(context);
        init();
    }
    ...
    private void updateSummary() {
        setSummary(str(!Settings.SPOTIFY_TOKEN.get().isBlank()
                ? "morphe_music_spotify_token_summary_set"
                : "morphe_music_spotify_token_summary"));
    }
    ...
    private void showDialog() {
        ...
        final boolean configured = !Settings.SPOTIFY_TOKEN.get().isBlank();

        LinearLayout content = new LinearLayout(context);
        content.setOrientation(LinearLayout.VERTICAL);

        TextView instruction = new TextView(context);
        instruction.setText(str("morphe_music_spotify_token_dialog_instruction"));
        ...
        tokenInput.setHint(str("morphe_music_spotify_token_dialog_hint"));
        tokenInput.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
        if (configured) {
            String currentToken = Settings.SPOTIFY_TOKEN.get();
        ...
        Pair<Dialog, LinearLayout> dialogPair = CustomDialog.create(
                context,
                str("morphe_music_spotify_token_title"),
                null,
                null,
                ...
                () -> {
                    ...
                    if (token.isEmpty()) {
                        Settings.SPOTIFY_TOKEN.resetToDefault();
                        updateSummary();
                        Utils.showToastShort(str("morphe_music_spotify_token_toast_cleared"));
                    } else {
                        Settings.SPOTIFY_TOKEN.save(token);
                        updateSummary();
                        Utils.showToastShort(str("morphe_music_spotify_token_toast_saved"));
                    }
                },
                null,
        ...
        Button getTokenBtn = CustomDialog.createButton(context, null,
                str("morphe_music_spotify_token_dialog_get_token"),
                () -> {
                    try {
                        ...
                        Logger.printException(() -> "SpotifyTokenPreference failed to open browser", ex);
                    }
                },
                false, false);
